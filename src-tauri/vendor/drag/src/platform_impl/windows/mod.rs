        ...
        match item {
            DragItem::Files(files) => {
                ...
                unsafe {
                    ...
                    // mini-tc fork: advertise BOTH copy and move so the drag is
                    // acceptable to any in-process target (wry's drop target returns
                    // fixed DROPEFFECT_COPY for CF_HDROP, so without COPY here the
                    // intersection is empty and the cursor shows forbidden for the
                    // entire webview). Explorer (and other out-of-process targets)
                    // will still pick MOVE on same-volume / COPY across volumes, the
                    // way Win32 drag is supposed to work.
                    let effect = match options.mode {
                        DragMode::Copy => DROPEFFECT_COPY | DROPEFFECT_MOVE,
                        DragMode::Move => DROPEFFECT_MOVE | DROPEFFECT_COPY,
                    };
